...
function ReportForm() {
    ...
    return (
      <div className="py-16 text-center">
        ...
        <p className="text-[13.5px] text-muted leading-[1.6] mb-6">
          Every report is read by the person running Stoop. {alsoBlock ? `We've also blocked ${otherName}, so you won't see or hear from them.` : ''}
        </p>
        <button type="button" onClick={() => router.push('/inbox')} className="btn btn-sm">Back to inbox</button>
      </div>
    );
  }
  ...
  return (
    <div className="py-10">
      ...
      <label htmlFor="report-details" className="block text-[12.5px] text-muted mb-1.5">Anything else? (optional)</label>
      {/* 16px below sm so mobile Safari does not zoom the page on focus and
          leave the form scrolled sideways; the design's 13.5px from sm up. */}
      <textarea
        id="report-details"
        value={details}
        ...
        className="w-full bg-card border border-[var(--border2)] rounded-[12px] px-3.5 py-2.5 text-[16px] sm:text-[13.5px] text-ink resize-none outline-none focus:border-accent/40 mb-5"
      />
      ...
      <div className="flex gap-2.5">
        <button type="button" onClick={() => router.back()} className="btn btn-sm btn-ghost">Cancel</button>
        <button type="button" onClick={submit} disabled={!reason || submitting} className="btn btn-sm">
          {submitting ? <span className="spinner" /> : 'Send report'}
        </button>
      </div>

      <p className="text-[11.5px] text-muted mt-6 leading-[1.6]">
        Every report is read. See our{' '}
...
// Nav and the main landmark sit OUTSIDE the Suspense boundary so the
// server-rendered fallback already has the one <main id="main"> the skip link
// points at, rather than gaining it only after hydration.
export default function ReportPage() {
  return (
    <>
      <Nav />
      <PageMain className="max-w-[560px] mx-auto px-6">
