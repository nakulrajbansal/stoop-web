...
export default function PlanCard({ plan }: { plan: Plan }) {
  ...
  return (
    <Link href={`/plan/${plan.id}`} className="plan-card block no-underline">
      ...
      <div className="flex items-center gap-1.5 text-[11.5px] text-muted mb-3 flex-wrap">
        ...
        {plan.when_time && (<><span className="opacity-30">·</span><span>{plan.when_time}</span></>)}
        ...
        <span
          className={`text-[10px] font-medium px-2 py-[3px] rounded-full ${
            isFull
              ...
              : 'text-accent bg-[rgba(200,71,42,0.09)]'
          }`}
        >
